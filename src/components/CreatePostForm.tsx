...
export const CreatePostForm = ({ accessToken }: { accessToken: string}) => {
  ...
  const submitHandler = async (formData: FormData) => {
    ...
    if (body?.split(' ').filter(word => word !== '').length! < 30) {
  ...
  return (
    <form action={submitHandler} className="mx-20">
      ...
      <div className="my-2">
        <div className="flex">
          <label htmlFor="body" className="mb-2 text-sm font-medium text-gray-900">Contents</label>
          <p className="text-red-700 text-sm font-medium mx-2">{ bodyError }</p>
        </div>
        <textarea id="body" name="body" rows={8} placeholder="Write your thoughts here..."
          className="form-textare p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg" />
      </div>
      
      <button type="submit" className="my-2 inline-flex items-center px-5 py-2.5 text-sm font-medium text-center text-white bg-blue-700 rounded-lg focus:ring-4 focus:ring-blue-200 hover:bg-blue-800">
        Publish post
      </button>
    </form>
    ...
    //           <textarea id="editor" rows={8} className="block w-full px-0 text-sm text-gray-800 bg-white border-0 focus:ring-0" placeholder="Write an article..." required />
    //       </div>
    //   </div>
